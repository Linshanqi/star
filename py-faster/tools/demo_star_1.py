...
import argparse
import csv
# path='/home/shelly/py-faster-rcnn/data/VOC2007/jpg'
...
NETS = {'vgg16': ('VGG16',
        ...
        'resnet':('ResNet-50',
                'ResNet-50_faster_rcnn_iter_80000.caffemodel')}
...
def vis_detections(im, class_name, dets, image_name, thresh=0.1):
    ...
    for i in inds:
        ...
        if(class_name=='__background__'):
            fw=open('./vgg/trial.txt','a',newline='')
            ...
            fw.write(str(image_name)+' '+class_name+' '+str(int(bbox[0]))+' '+str(int(bbox[1]))+' '+str(int(bbox[2]))+' '+str(int(bbox[3]))+' '+str(score)+'\n')
            fw.close()
        elif(class_name=='star'):
            fw=open('./vgg/trial.txt','a')
            # csv_write=csv.writer(fw,dialect='excel')
            # stu=[str(image_name),class_name,str(int(bbox[0])),str(int(bbox[1])),str(int(bbox[2])),str(int(bbox[3])),str(int(bbox[3])),str(score)]
            # csv_write.writerow(stu)
            fw.write(str(image_name)+' '+class_name+' '+str(int(bbox[0]))+' '+str(int(bbox[1]))+' '+str(int(bbox[2]))+' '+str(int(bbox[3]))+' '+str(score)+'\n')
    ...
    plt.margins(0,0)
    plt.draw()
    # plt.savefig("/home/shelly/py-faster-rcnn/data/trial/"+image_name)
...
def demo(net, image_name):
    ...
    # Load the demo image
    im_file = os.path.join('./data/demo/testB', image_name)
...
if __name__ == '__main__':
    ...
    for im_name in os.listdir(ss):
        ...
        demo(net, im_name)
        count=count+1
        plt.savefig("./data/demo/testvgg/" + im_name,dpi=300)